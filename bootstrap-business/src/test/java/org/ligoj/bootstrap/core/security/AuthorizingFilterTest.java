...
class AuthorizingFilterTest extends AbstractBootTest {
	...
	void doFilterAttachedAuthority3() throws Exception {
		...
		final var servletContext = mock(ServletContext.class);
		when(servletContext.getContextPath()).thenReturn("/context");
		when(request.getRequestURI()).thenReturn("/context/rest/match");
	...
	void doFilterViaUserNotAdmin() throws Exception {
		...
		when(request.getQueryString()).thenReturn("query");
		when(request.getMethod()).thenReturn("GET");
		when(request.getHeader("x-api-via-user")).thenReturn(DEFAULT_USER);
		final var outputStream = mock(ServletOutputStream.class);
		final var response = mock(HttpServletResponse.class);
		when(response.getOutputStream()).thenReturn(outputStream);
		authorizingFilter.setServletContext(servletContext);
	...
	private void attachRole(final String... roleNames) {
		...
		when((Collection) SecurityContextHolder.getContext().getAuthentication().getAuthorities()).thenReturn(authorities);
